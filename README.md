...
- **Overnight Quiet Mode** — optional (off by default) battery-only feature that skips the display refresh entirely during a configurable overnight window and sleeps directly until the window ends, instead of waking every sleep-duration interval all night. Toggle via the "Overnight Quiet Mode" switch; configure "Overnight Begin Hour" / "Overnight End Hour" (0–23, default 1–6 AM) in the HA device config
...
An optional battery-only optimization for further reducing power use overnight, when nobody is watching the display. When the "Overnight Quiet Mode" switch is on and the device wakes on battery inside the configured hour window (`Overnight Begin Hour`–`Overnight End Hour`), it skips the display refresh entirely and goes straight back into PMIC full-shutdown, computing exactly how long to sleep so it wakes right as the window ends — rather than waking every `On Battery Sleep Duration` interval all night for no visible benefit.
