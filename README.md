...
Integrates with Home Assistant to display a dashboard with outdoor temperature, room temperature/humidity, battery level, and WiFi signal strength. Designed for battery-powered use with PMIC full-shutdown sleep (~20–22 day measured battery life at 30-min intervals, three confirmed discharge runs; ~26 days confirmed with Overnight Quiet Mode at 30-min intervals).
...
- **PMIC full-shutdown sleep** — configurable sleep cycle (default 20 min on battery); measured **~20–22 day battery life** at 30-min intervals (confirmed runs: Jun 2026 → 19.97 days, Jul 2026 → ~22.25 days); with Overnight Quiet Mode, **~26.3 days** confirmed (Jul–Aug 2026, 30-min intervals, +18%). The 20-min default interval has not been independently re-tested — the earlier ~10-day estimate is stale. Uses M5PM1 internal wake timer (`SYS_CMD_OFF` + regs 0x38–0x3D) — cuts all power to ~92 µA standby, matching the product datasheet spec. ESP32 deep sleep was tested empirically and found ineffective (~5–10 mA sleep current regardless of sleep interval; ~1.8 day battery life). Wake is via PMIC timer (automatic) or physical power button S4.
...
- **Configurable intervals** — "On Battery Sleep Duration" and "On USB Refresh Interval" sliders (5–120 min, step 5) available in the HA device config; device must be awake for changes to be delivered. Any setting changed via HA is force-flushed to flash before every PMIC shutdown, so it survives the next power cycle even if you change it right before the device sleeps
- **Overnight Quiet Mode** — optional (off by default) battery-only feature for squeezing further life out of a charge: nobody is looking at the display while they're asleep, so each wake during the configured overnight window skips the display refresh entirely and sleeps directly until the window ends, instead of burning a refresh cycle every sleep-duration interval all night for a screen no one will see. Toggle via the "Overnight Quiet Mode" switch; configure "Overnight Begin Hour" / "Overnight End Hour" (0–23, default 1–6 AM) in the HA device config; **~26.3 days** battery life confirmed at 30-min intervals with the default 1–6 AM window (+18% over baseline)
...
**Why not ESP32 deep sleep:** Empirical discharge testing showed that ESP32 deep sleep provides essentially no power reduction on this hardware. Comparing 5-min and 30-min sleep intervals yielded only 1.11× difference in drain rate (should be ~6× if wake cycles dominated). Implied sleep current during ESP32 deep sleep is ~5–10 mA — the IT8951E EPD controller and PMIC monitoring circuits remain active regardless. Battery life with ESP32 deep sleep: ~1.8 days. With PMIC full-shutdown: **~20–22 days measured** at 30-min intervals (confirmed runs: Jun 10–30 2026 → 19.97 days; Jul 1–23 2026 → ~22.25 days — both supersede an earlier ~15-day estimate extrapolated from a partial-rate sample). The 20-min default interval's earlier ~10-day estimate was derived from that same superseded sample and hasn't been independently re-confirmed.

With Overnight Quiet Mode enabled (default 1–6 AM window), a confirmed full discharge run (Jul 25–Aug 20 2026, 30-min intervals) measured **~26.3 days** — approximately **+18%** over the 22.25-day baseline. The 20-min-with-Quiet-Mode figure hasn't been independently re-tested.
...
### Overnight Quiet Mode

An optional optimization for extending battery life further: each display refresh costs current draw and awake time, and overnight — when nobody is watching the display — that cost buys nothing. When the "Overnight Quiet Mode" switch is on and the device wakes on battery inside the configured hour window (`Overnight Begin Hour`–`Overnight End Hour`), it skips the display refresh entirely and goes straight back into PMIC full-shutdown, computing exactly how long to sleep so it wakes right as the window ends — rather than waking every `On Battery Sleep Duration` interval all night for no visible benefit.

This uses the existing Home Assistant time sensor (`ha_time`) as the time source, so the device still connects to WiFi every wake (HA time requires it) — only the ~15–19s EPD refresh and its associated current draw are skipped. The window correctly handles midnight-spanning ranges (e.g. 22–6) and falls back to the normal full-refresh cycle if the configured window is degenerate (`start == end`) or if less than one normal sleep interval remains before the window ends. Manual wakes via the physical power button S4 during the window are indistinguishable from a timer wake at the firmware level — the display will simply continue showing whatever was last rendered before the window began.

A confirmed full discharge run (Jul 25–Aug 20 2026, 30-min intervals, default 1–6 AM window) measured **~26.3 days** — approximately **+18%** over the 22.25-day baseline without Quiet Mode.

A more aggressive version — skipping the WiFi connection too, using the onboard RTC (RX8130CE, I2C 0x32) as the time source instead of Home Assistant — is a planned future enhancement.
