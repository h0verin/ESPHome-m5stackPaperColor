...
Integrates with Home Assistant to display a dashboard with outdoor temperature, room temperature/humidity, battery level, and WiFi signal strength. Designed for battery-powered use with PMIC full-shutdown sleep (~20 day measured battery life at 30-min intervals).
...
**Why not ESP32 deep sleep:** Empirical discharge testing showed that ESP32 deep sleep provides essentially no power reduction on this hardware. Comparing 5-min and 30-min sleep intervals yielded only 1.11× difference in drain rate (should be ~6× if wake cycles dominated). Implied sleep current during ESP32 deep sleep is ~5–10 mA — the IT8951E EPD controller and PMIC monitoring circuits remain active regardless. Battery life with ESP32 deep sleep: ~1.8 days. With PMIC full-shutdown: **~20 days measured** at 30-min intervals (full discharge run, off USB to low-battery cutoff, Jun 10–30 2026 — supersedes an earlier ~15-day estimate that was extrapolated from a partial-rate sample rather than a complete run). The 20-min default interval's earlier ~10-day estimate was derived from that same superseded sample and hasn't yet been independently re-confirmed with its own full discharge run. Enabling Overnight Quiet Mode with its default 5-hour window (1–6 AM) eliminates most of the wake cycles that would otherwise occur during that span each night — applying the same ~20% per-cycle savings logic to the newly confirmed 30-min figure projects roughly **~24 days at 30-min intervals**; the 20-min-with-Quiet-Mode figure isn't re-derived here pending a fresh 20-min baseline. None of the Quiet Mode figures have been confirmed with a dedicated discharge run of their own.
